...
void cPlayer::addToScan(const cUnit& unit)
{
	...
	const int size = unit.getCellSize();
...
//------------------------------------------------------------------------------
void cPlayer::updateScan(const cUnit& unit, const cPosition& newPosition, int newSize)
{
	assert(!unit.isDisabled());

	const int oldSize = unit.getCellSize();
...
cVehicle& cPlayer::addNewVehicle (const cPosition& position, const sVehicleDataPtr& unitData, unsigned int uid)
{
	...
	const cDynamicUnitData* ddata = getUnitDataCurrentVersion(unitData->ID);
	auto vehicle = std::make_shared<cVehicle> (unitData, ddata, this, uid);
...
cBuilding& cPlayer::addNewBuilding (const cPosition& position, const sBuildingDataPtr& unitData, unsigned int uid)
{
	...
	const cDynamicUnitData* ddata = getUnitDataCurrentVersion(unitData->ID);
	auto building = std::make_shared<cBuilding>(unitData, ddata, this, uid);
