...
int cPathCalculator::calcNextCost(const cPosition& source, const cPosition& destination, const cVehicle* vehicle, const T* map)
{
	...
	if (vehicle->getStaticUnitData().factorAir > 0)
	{
		if (source.x() != destination.x() && source.y() != destination.y())
			return (int)(4 * 1.5f * vehicle->getStaticUnitData().factorAir);
		else
			return (int)(4 * vehicle->getStaticUnitData().factorAir);
	}
	const cBuilding* building = map->getField(destination).getBaseBuilding();
	// moving on water will cost more
	if (map->isWater(destination) && (!building || (building->getStaticUnitData().surfacePosition == cStaticUnitData::SURFACE_POS_BENEATH_SEA || building->getStaticUnitData().surfacePosition == cStaticUnitData::SURFACE_POS_ABOVE)) && vehicle->getStaticUnitData().factorSea > 0)
		costs = (int)(4 * vehicle->getStaticUnitData().factorSea);
	else if (map->isCoast(destination) && !building && vehicle->getStaticUnitData().factorCoast > 0)
		costs = (int)(4 * vehicle->getStaticUnitData().factorCoast);
	else if (vehicle->getStaticUnitData().factorGround > 0)
	...
	if (building && building->getStaticUnitData().modifiesSpeed != 0)
